    ...
    it("This Module is for Clinic appointment", () => {
      cy.url()
        ...
        .then(() => {
          ...
          cy.then(() => {
            cy.contains("Online Appointment").click({force:true});
            cy.get(".page-content");
            cy.get("[placeholder=\"Enter Speciality Name\"]").click({force:true}).type("Dermatology");
           cy.get("button").contains("View Doctors").click().then(()=>{
           cy.get("[placeholder=\"Doctor Name\"]").click({force:true}).type("Ahmeddd");
           cy.get("button").contains("Search").click();
           cy.get("tr").eq(0);
           cy.xpath("//*[@id=\"doctorTable\"]/tbody/tr[1]/td[6]/a").click();
          });
          cy.get(".page-content");
          ...
          cy.get(".react-calendar__navigation__next-button").click().then(()=>{
            ...
            cy.get("button.react-calendar__month-view__days__day").contains(25).click();
            cy.get(".col-md-12");
                 cy.get("button");
